...
public class Logger {
  ...
  /**
   * Enables or disables warnings for cyclic references of loggables. Kotlin users
   * will likely want
   * to disable these, as kotlin singletons compile to cycles in the bytecode
   * which cannot be
   * explicitly broken with {@link Log.Exclude}.
   *
   * @param enabled Whether to print a warning on configuration when cyclic
   *                references of loggables
   *                are detected.
   */
  ...
  /**
   * Configure shuffleboard logging for the robot. Should be called after all
   * loggable objects have
   * been instantiated, e.g. at the end of robotInit. Tabs for logging will be
   * separate from tabs
   * for config.
   *
   * @param rootContainer The root of the tree of loggable objects - for most
   *                      teams, this is
   *                      Robot.java. To send an instance of Robot.java to this
   *                      method from robotInit, call
   *                      "configureLogging(this)" Loggable fields of this object
   *                      will have their own shuffleboard
   *                      tabs.
   */
  ...
  /**
   * Configure shuffleboard config for the robot. Should be called after all
   * loggable objects have
   * been instantiated, e.g. at the end of robotInit. Tabs for config will be
   * separate from tabs for
   * logging.
   *
   * @param rootContainer The root of the tree of loggable objects - for most
   *                      teams, this is
   *                      Robot.java. To send an instance of Robot.java to this
   *                      method from robotInit, call
   *                      "configureConfig(this)" Loggable fields of this object
   *                      will have their own shuffleboard
   *                      tabs.
   */
  ...
  /**
   * Configure shuffleboard logging and config for the robot. Should be called
   * after all loggable
   * objects have beeen instantiated, e.g. at the end of robotInit. Config and
   ...
   *                      Robot.java. To send an instance of Robot.java to this
   *                      method from robotInit, call
   *                      "configureLoggingAndConfig(this)" Loggable fields of
   *                      this object will have their own
   *                      shuffleboard tabs.
   * @param separate      Whether to generate separate tabs for config and
   *                      logging. If true, log widgets
   *                      will be placed in tabs labeled "Log", and config widgets
   *                      will be placed in tabs labeled
   *                      "Config".
   */
  ...
  /**
   * Configures logging to send values over NetworkTables, but not to add widgets
   * to Shuffleboard .
   ...
   * @param rootName      Name of the root NetworkTable. oblog.Loggable fields of
  ...
  /**
   * Registers a new entry. To be called during initial logging configuration for
   * any value that
   * will change during runtime. Mostly for internal use, but can be used by
   * advanced users in
  ...
  /**
   * Configures logging or config for the robot. Begins at the specified root
   * container and recurses
   * down the tree of Loggable objects. The root container itself does have a tab,
   * so that fields
   * located in it can be logged, but is not required to implement Loggable.
   *
   * @param logType       The type of logging to perform (either logging or
   *                      config)
   * @param separate      Whether logging and config should be given separate tabs
   * @param rootContainer The root of the tree of loggable objects. Does not need
   *                      to be Loggable
   *                      itself.
   * @param shuffleboard  The shuffleboard instance to use; wrapped to allow
   *                      NT-only mode
  ...
  /**
   * Maps config annotations to the appropriate methods for processing their
   * annotated setters. Acts
      ...
      entry(
          ...
          (setter, rawParams, bin, nt, name, isBoolean) -> {
            if (isBoolean) {
              ...
              nt.addEntryListener(
                  entry,
                  (entryNotification) -> setterRunner.execute(
  ...
  /**
   * Maps log annotations to the appropriate method for processing their annotated
   * fields. Acts
   * essentially as a switch statement on the annotation type,
   */
  ...
  /**
   * Performs data binding for declared fields and methods of the given class
   * based on the present
   * config annotations.
   *
   * @param loggable          The object to configure
   * @param loggableClass     The class to configure - necessary because the
   *                          algorithm recurses on
   *                          subclasses
   * @param bin               The shuffleboard container to place widgets in
   * @param nt                The networktable instance
   * @param registeredFields  The list of already-processed fields, to avoid
   *                          double-processing during
   *                          recursion
   * @param registeredMethods The list of already-processed methods, to avoid
   *                          double-processing
   *                          during recursion
   */
  private static void configFieldsAndMethods(
    ...
    // Process fields; configurable fields are all Sendables or have the methodName
    // parameter
  ...
  private static Map<Class<? extends Annotation>, BiFunction<Field, Annotation, Method>> methodHandler = Map.ofEntries(
      entry(
          Config.class,
          (field, config) -> getMethod(
  ...
  private static void handleMultiArgSetter(
    ...
    for (Config annotation : annotations) {
      if (annotation != null) {
        ...
        // Create persistent list of params that can be shared by all listeners, since
        // the method
        // requires
  ...
  /**
   * Performs logging for declared fields and methods of the given class based on
   * the present log
   * annotations.
   *
   * @param loggable          The object to configure
    ...
    for (Method method : methods) {
      ...
      for (Class type : logHandler.keySet()) {
        ...
        for (Annotation annotation : method.getAnnotationsByType(type)) {
          ...
          FieldProcessor process = logHandler.get(annotation.annotationType());
          if (process != null) {
            // Process the getter (uses "processField" because the requirements are the
            // same)
            process.processField(
                () -> {
                  try {
  ...
  /**
   * Configures logging and config for the given Loggable class. Recurses first on
   * all loggable
   ...
   * @param loggableClass     The class of the object being configured (necessary
   *                          due to superclass
   *                          recursion)
   * @param loggedFields      A list of all previously-handled Loggable fields, to
   *                          avoid double-counting
   *                          during recursion
   * @param registeredFields  A list of all previously-handled annotated fields,
   *                          to avoid
   *                          double-counting during recursion
   * @param registeredMethods A list of all previously-handled annotated methods,
   *                          to avoid
   *                          double-counting during recursion
   * @param shuffleboard      The shuffleboard instance (wrapped to support
   *                          NT-only mode)
   * @param nt                The networktable instance
   * @param parentContainer   The container of this loggable's parent in the graph
   *                          of loggable objects
   * @param ancestors         A list of ancestors, to detect cyclic references of
   *                          loggables
   */
  private static void logLoggable(
    ...
    // Set up the container depending on the logging type and log the fields and
    // methods
    ShuffleboardContainerWrapper bin;
    switch (logType) {
      case LOG:
        // Tab if there is no parent container (i.e. the loggable is in the
        // rootcontainer)
    ...
    // Set up call for recursion on Loggable fields with parameters from current
    // Loggable
    Consumer<Loggable> log = (toLog) -> logLoggable(
        logType,
        separate,
    ...
    for (Field field : loggableClass.getDeclaredFields()) {
      ...
      // Only proceed if loggable, included, not already logged, and does not cause a
      // cycle
  ...
  /**
   * Checks if a field is in the given list of ancestors, printing a warning if it
   * is as this means
  ...
  /**
   * Checks if a given field is a Loggable class, or else an array or collection
   * (as these might
  ...
  /**
   * Returns a "defaulted" config annotation, because Java is gross and doesn't
   * include a built-in
   * way to do this.
   *
  ...
  /**
   * Returns a "defaulted" log annotation, because Java is gross and doesn't
   * include a built-in way
   * to do this.
   *
  ...
  /**
   * Gets a config annotation from a parameter. Returns a "defaulted" annotation
   * if none is present.
   *
  ...
  /**
   * Gets a new supplier from calling the given method name on the output of the
   * provided supplier.
   * If methodName is "DEFAULT", the original supplier is provided.
   *
   * @param supplier   The original supplier
   * @param methodName The name of the method to call on the output of the
   *                   original supplier;
